    ...
    async invoke(input: NormalizedProviderInput): Promise<NormalizedProviderOutput> {
      if (!apiKey) {
        throw new AgentGuardProviderError(
          "auth",
          "Missing OPENAI_API_KEY. Set OPENAI_API_KEY to run OpenAI provider tests.",
        );
      }
      ...
      try {
        ...
        if (response.status === 401 || response.status === 403) {
          throw new AgentGuardProviderError(
            "auth",
            `OpenAI authentication failed (HTTP ${response.status}). Check OPENAI_API_KEY.`,
          );
        }
        ...
        throw new AgentGuardProviderError(
          "network",
          `OpenAI request failed due to a network/runtime error: ${reason}`,
          { retryable: true },
        );
      } finally {
